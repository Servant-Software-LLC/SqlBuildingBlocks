...
public class NegativeTests
{
    ...
    {
        // 1. Incomplete WHERE -- WHERE keyword followed by nothing
        new object[] { "SELECT * FROM Customers WHERE" },

        // 2. Unclosed parens in expression
        new object[] { "SELECT (1 + 2 FROM Customers" },

        // 3. Empty IN list
        new object[] { "SELECT * FROM Customers WHERE x IN ()" },

        // 4. Malformed FETCH FIRST -- FETCH FIRST without ROWS or count
        new object[] { "SELECT * FROM Customers FETCH FIRST" },

        // 5. Dangling comma in SELECT list (issue #167) -- now rejected by the
        //    AnsiSQL grammar because FROM/INTO are reserved words and cannot be
        ...
        // 10. Trailing operator -- WHERE x = with no RHS
