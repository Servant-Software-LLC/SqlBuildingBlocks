...
/// ≥50 cases per dialect, organised by category. The shared audit doc lives at
...
/// not in this catalog — those are findings rather than enforced behavior. Bare
/// <c>SELECT *</c> with no FROM clause is well-formed per the grammar (FROM is
/// optional) and is not a finding.
/// </summary>
public class NegativeTests
{
    ...
    // ── Dialect-specific (MySQL) ──────────────────────────────────────────────
    public static IEnumerable<object[]> DialectSpecific => new[]
    {
        new object[] { "SELECT * FROM Customers LIMIT" },     // bare LIMIT
        new object[] { "SELECT * FROM Customers LIMIT 5," },  // LIMIT trailing comma
    };
